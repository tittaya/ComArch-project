from Type_Code import J_type,I_type,O_type,R_type,gen_32twoCom

def Assembler(lineSplit,mem,PC,label_addr):                                  
        if lineSplit[1] == 'add' or lineSplit[1] == 'nand'  :                          #เช็คตาม Type 
            mem[PC] = int(R_type(lineSplit),2)                                 #go to Type_code.py ได้เลขฐานสองมาทำเป็นเลขฐานสิบเก็บใน mem
        elif  lineSplit[1] == 'lw' or lineSplit[1] == 'sw' or lineSplit[1] == 'beq' :   
            mem[PC] = int(I_type(lineSplit,PC,label_addr),2)
        elif lineSplit[1] == 'halt' or lineSplit[1] == 'noop' :
            mem[PC] = int(O_type(lineSplit),2)
        elif lineSplit[1] == 'jalr':
            mem[PC] = int(J_type(lineSplit),2)
